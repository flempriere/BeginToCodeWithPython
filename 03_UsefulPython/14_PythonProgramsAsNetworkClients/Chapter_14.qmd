...
- Python's socket module provides methods to find the IP of the computer running the python program (see [Receiver.py](./Examples/01_Receiver/Receiver.py))
...
- You should see the [sender program](./Examples/02_Sender/Sender.py) periodically report that it is sending a message
...
- Weather snaps from [Chapter 5](../../01_ProgrammingFundamentals/05_MakingDecisions/Chapter_05.qmd#make-something-happen-weather-helper) also fetches an XML document from the internet
...
- See the [XML Wikipedia page](https://en.wikipedia.org/wiki/XML) for more information
...
We can combine the url request code and the RSS walk code into one program that fetches an XML RSS feed from a site, then parses its contents. You can see it in [RSSFeed.py](./Examples/05_RSSFeed/RSSFeed.py)
...
We'll stick with our typical example of Seattle for now. If we look at the appropriate URL for [Seattle](https://forecast.weather.gov/MapClick.php?lat=47.61&lon=-122.33&unit=0&lg=english&FcstType=dwml)
...
1. *Do wireless devices use a different version of the Internet from wired ones?*
    ...
    - This is analogous to how we had a GUI and Shell-based class for our Fashion Shop program in [Chapter 13](../13_PythonAndGraphicalUserInterfaces/Chapter_13.qmd#code-analysis-complete-fashion-shop-program)
