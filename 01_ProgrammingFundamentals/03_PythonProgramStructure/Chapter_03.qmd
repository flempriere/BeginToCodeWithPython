...
*Write and run a new file ([using_print3.py](./Examples/02_UsingPrint/using_print3.py)) containing the above line and check the output matches the above.*
...
*You should see the program print the first line, pause for about* $5$ *seconds, then print the second line. The above shell should show the code after all lines have run*
...
```{python}
#| echo: false
# cheating a little by having the actual executed code omit the sleeps so it doesn't take forever to render
import time
...
- The `snaps` library, is a python file contained in the provided book [sample code](https://github.com/Begintocodewithpython/samples), in each chapter folder with examples.
...
- *The above creates a window with red text saying 'Hello World'.*
  - *We've added the `sleep` so the window doesn't automatically close - see discussion about [Delaying the end of a program](#delay-the-end-of-a-program)*
...
import random
print("The first number is: ", random.randint(1,10))
print("Will the next be higher or lower?")
print("The second number is: ", random.randint(1, 10))
```
