...
7. *What will the following program print?*
   ...
   ```
    ...
    - At each iteration we print `"Inside Loop"` *
    and increase the value of `count` by $1$
...
*One of the examples [in the above question set](#code-analysis-investigating-the-while-construction) involved a count***up** *to* $5$*. Implement a program that counts down from* $10$ *to* $0$ *over* $10$ *seconds*
...
Our solution (given in [UserSelectedTimesTableTutor.py](./Exercises/05_UserSelectedTimesTableTutor.py/UserSelectedTimesTableTutor.py)) prompts the user to enter a number between $2$ - $12$ inclusive, and then validates that the input is in the range.

A `try`, `except` block is used to catch any invalid input type, and the whole thing is wrapped in a `while True` block that only ends once a valid integer in the range $2$ - $12$ is received through the use of a `break` statement.
