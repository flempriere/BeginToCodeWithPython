...
Observe that first we wrap all of the code in a `while` loop, and introduce a boolean flag `run_program` initially set to `True` to flag if the program continues to run at each loop iteration. When a user enters a number we first check if it corresponds to a ride number and if not, we set the program to quit on the next loop iteration by setting `run_program` to `False`. We now change the original `if ride_number == 1` to and `elif` so it is only checked if we know the ride_number is valid
...
This is a straightforward exercise, we set up our counter value to $10$ and use the appropriate loop expression (here `time_left >= 0`) to ensure that $0$ is included in the countdown.
