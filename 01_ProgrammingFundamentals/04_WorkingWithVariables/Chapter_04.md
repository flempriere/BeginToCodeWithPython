...
1. `total = 0`
    - *No output*
    - *The assignment of `0` to `total` doesn't generate any output*
2. `total`
    - `0` *output*
...
### [Self-Timer Party Game](./Examples/02_SelfTimer/selfTimer.py)

*Consider the [Nerves of Steel](../03_PythonProgramStructure/Chapter_03.md#exercise-34-nerves-of-steel) exercise from [Chapter 3](../03_PythonProgramStructure/Chapter_03.md). In the game, players must remain standing right up to the moment before they think a random timer will expire*
...
### Exercise 4.1: Typing Errors and Testing
...
3. *The statement below also contains a misspelling of the variable* `total`*. However, this time the name on the right-hand side of the equals is mispelled. What will happen when this program runs?*
    ...
    - In this case, `Total` is not defined as a variable, so there is no meaningful way to interpret a value. Hence we would expect an error. Since the error is that `Total` doesn't exist, we would expect a `NameError`. The actual output is,
...
- See [Example: Working with Variables](#working-with-variables)
...
- See [Ex 4.1](#exercise-41-typing-errors-and-testing)
- See [Example: Self-Timer Party Game](#self-timer-party-game)
...
- See [Example: Text and Numeric Variables](#text-and-numeric-variables)
